...
class GeneticAlgo:
    ...
    def mutate(self, individuals_list):
        
        selelected_individual = individuals_list[random.randint(0, len(individuals_list) - 1)].copy();
        
        # for i in range(random.randint(1, int(len(selelected_individual) / 4) + 1)):
        for i in range(1):
            ...
            while not mutation_accepted:
                ...
                value_at_mutation_index = selelected_individual[mutation_index];
                
                #swap this value to index where the value is there
                new_value = random.randint(0, self.total_genes - 1);
                
                new_value_index = selelected_individual.index(new_value);
                
                if new_value_index != value_at_mutation_index:
                    selelected_individual[mutation_index] = new_value;
                    selelected_individual[new_value_index] = value_at_mutation_index;
                    
                    mutation_accepted = True;

        return selelected_individual;
    ...
    def optimize(self):
        ...
        for i in range(self.total_generations):
            ...
            if random_number <= self.mutation_chance:
                #mutate any of population members other than top
                mutated_individual = self.mutate(self.population_list[2:]);
                
                if mutated_individual not in self.population_list:
                    self.population_list.append(mutated_individual);
            ...
            if child not in self.population_list:
                ...
                self.population_list =  self.population_list[0 : self.total_population];
                self.fitness_list =  self.fitness_list[0 : self.total_population];
